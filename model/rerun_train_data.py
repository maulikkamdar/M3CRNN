...
def run_new_data(test_df, g, filename = "validation_set_preds"):
    te = BucketedDataIterator(test_df)
    step, accuracy = 0, 0
    te_accs, losses = [], []
    validation_set_df = pd.DataFrame()
    current_epoch = 0
    while te.epochs == 0:
        ...
        accuracy_, preds, scores = sess.run([g['accuracy'], g['preds'], g['scores']], feed_dict=feed)
        #print (scores)
        bds = pd.concat([batch[1], batch[2], batch[3], pd.Series(preds).to_frame(name="preds"), pd.Series(scores[:,1]).to_frame(name="scores")], axis=1)
        if validation_set_df.shape[0] == 0: validation_set_df = bds
        else: validation_set_df = pd.concat([validation_set_df, bds], axis=0)
        accuracy += accuracy_
        te_accs.append(accuracy / step)
    print("Validation Set Accuracy", current_epoch, "- val:", te_accs[-1])
    sys.stdout.flush()
    validation_set_df.to_csv(filename + ".tsv", sep="\t", index=None)
...
val_df = pd.read_csv("rnn_train_new_annotated.tsv", sep="\t")
val_df = val_df[val_df["is_valid"] == 1]
val_df = pd.concat([val_df, val_df["meth_stat"].apply(lambda x: 1 if x > 0 else 0).to_frame(name=output_identifer)], axis=1)


test_df = pd.read_csv("test_new.tsv", sep="\t")
...
val_rev_df = deepcopy(val_df)
test_rev_df = deepcopy(test_df)
val_rev_df["shahash"] = val_rev_df["shahash"] + "_rev"
...
run_new_data(val_df, g, "run14_2_epoch10_final_train_set_preds_istrainingTrue")
#run_new_data(test_df, g, "best_test_set_preds")
